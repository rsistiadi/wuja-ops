      ...
      {!selected ? (
        ...
        <div className="flex-1 flex flex-col px-5 py-6" style={{ background: C.inkSoft }}>
          <div className="rounded-2xl p-5 flex flex-col items-center" style={{ background: C.ink, border: `1px solid ${C.inkLine}` }}>
            ...
            <div style={{ fontFamily: "Fraunces, serif", color: C.parchment, fontSize: 20, fontWeight: 600, marginTop: 12 }}>{selected.full_name}</div>
...
function Row({ label, value, color }) {
  return (
    <div className="flex justify-between">
      ...
      <span style={{ fontFamily: "JetBrains Mono, monospace", color: color || C.parchment, fontSize: 12.5 }}>{value}</span>
    </div>
  );
}
