...
export default function BottomTabBar({ mode, setMode, visibleTabs }) {
  ...
  return (
    <div className="flex items-stretch flex-shrink-0" style={{ borderTop: `1px solid ${C.inkLine}`, background: C.ink }}>
      {tabs.map((t) => {
        const Icon = t.icon;
        ...
        return (
          <button key={t.key} onClick={() => setMode(t.key)} className="flex-1 flex flex-col items-center justify-center gap-1"
            ...
            <Icon size={18} color={active ? C.gold : C.ink40} />
            <span style={{ fontSize: 9.5, fontWeight: 700, color: active ? C.gold : C.ink40 }}>{t.label}</span>
          </button>
        );
      })}
    </div>
