...
export function TopBar({ title, subtitle, onBack, accent }) {
  return (
    <div className="flex items-center gap-3 px-5 pt-6 pb-4" style={{ background: C.ink }}>
      ...
      <div>
        <div style={{ fontFamily: "Fraunces, serif", color: C.parchment, fontSize: 20, fontWeight: 600, lineHeight: 1.1 }}>{title}</div>
        {subtitle && <div style={{ fontFamily: "Inter, sans-serif", color: C.ink60, fontSize: 13.5, marginTop: 2 }}>{subtitle}</div>}
      </div>
    </div>
  );
}
...
export function PrimaryButton({ children, onClick, icon: Icon, tone = "gold", disabled, type = "button" }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} className="w-full flex items-center justify-center gap-2 rounded-xl"
      style={{ fontFamily: "Inter, sans-serif", background: disabled ? C.inkLine : bg, color: disabled ? C.ink60 : C.ink, fontWeight: 700, fontSize: 16.5, padding: "14px 16px", opacity: disabled ? 0.6 : 1, border: "none", cursor: disabled ? "default" : "pointer" }}>
...
export function GhostButton({ children, onClick, icon: Icon }) {
  return (
    <button onClick={onClick} className="w-full flex items-center justify-center gap-2 rounded-xl"
      style={{ fontFamily: "Inter, sans-serif", background: "transparent", color: C.parchment, fontWeight: 600, fontSize: 15.5, padding: "12px 16px", border: `1px solid ${C.inkLine}`, cursor: "pointer" }}>
...
export function Dropdown({ value, onChange, options, label }) {
  return (
    <div>
      {label && <div style={{ color: C.ink60, fontSize: 12.5, fontWeight: 600, marginBottom: 4 }}>{label}</div>}
      <div className="relative">
        <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-xl outline-none"
          style={{ background: C.inkSoft, border: `1px solid ${C.inkLine}`, color: C.parchment, fontSize: 14.5, fontWeight: 600, padding: "11px 34px 11px 14px", appearance: "none" }}>
...
export function StatusPill({ reg }) {
  ...
  return <span className="rounded-full" style={{ fontSize: 12.5, fontWeight: 700, padding: "4px 9px", background: `${s.color}22`, color: s.color }}>{s.label}</span>;
}
