  ...
  return (
    <div className="flex-1 px-5 py-5" style={{ background: C.inkSoft }}>
      <div className="rounded-xl p-4 flex items-center justify-between" style={{ background: C.ink, border: `1px solid ${C.inkLine}` }}>
        <div className="pr-3">
          <div style={{ color: C.parchment, fontSize: 13.5, fontWeight: 600 }}>Allow skipping photo capture</div>
          <div style={{ color: C.ink60, fontSize: 11.5, marginTop: 4 }}>Super Admin only. When off, "Skip for now" disappears desk-wide.</div>
          {!isSuperAdmin && <div style={{ color: C.gold, fontSize: 11, marginTop: 6 }}>You're an Admin, not a Super Admin — this toggle is read-only for you.</div>}
        </div>
