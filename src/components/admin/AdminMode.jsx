      ...
      <div className="px-5 pb-3 grid grid-cols-4 gap-1.5" style={{ background: C.ink }}>
        {tabs.map((t) => { const Icon = t.i; const active = tab === t.k; return (
          <button key={t.k} onClick={() => setTab(t.k)} className="relative flex flex-col items-center justify-center gap-1 rounded-lg"
            style={{ padding: "8px 0", fontSize: 10, fontWeight: 700, background: active ? C.gold : C.inkSoft, color: active ? C.ink : C.ink60, border: `1px solid ${active ? C.gold : C.inkLine}`, cursor: "pointer" }}>
            <Icon size={14} />{t.l}
            {!!t.badge && <span style={{ position: "absolute", top: -5, right: -5, background: C.alert, color: "#fff", borderRadius: 999, fontSize: 9, fontWeight: 700, width: 16, height: 16, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "JetBrains Mono, monospace" }}>{t.badge}</span>}
          </button>
        ); })}
      </div>
