...
export default function AdminItems() {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3" style={{ background: C.inkSoft }}>
      ...
      {items === null && <div style={{ color: C.ink40, fontSize: 13.5 }}>Loading…</div>}
      {items?.map((item) => (
        <div key={item.id} className="rounded-xl p-4" style={{ background: C.ink, border: `1px solid ${C.inkLine}` }}>
          <div className="flex items-center justify-between mb-2">
            <span style={{ color: C.parchment, fontSize: 15, fontWeight: 700 }}>{item.name}</span>
            <button onClick={() => setEditVariant({ item, variant: null })} className="flex items-center gap-1" style={{ background: "none", border: "none", color: C.gold, fontSize: 12, fontWeight: 700, cursor: "pointer" }}><Plus size={13} /> Variant</button>
          </div>
          {(item.merch_item_variants || []).length === 0 && <div style={{ color: C.ink40, fontSize: 12.5 }}>No variants yet — add one to set a price and stock.</div>}
          {(item.merch_item_variants || []).map((v) => (
            <div key={v.id} className="flex items-center justify-between py-2" style={{ borderTop: `1px dashed ${C.inkLine}` }}>
              <div>
                <div style={{ color: C.parchment, fontSize: 13.5, fontWeight: 600 }}>{v.variant_label || "— no variant —"}</div>
                <div style={{ color: C.ink40, fontSize: 11.5, marginTop: 2 }}>{formatIDR(v.price)} · {v.stock_available} available{v.stock_damaged > 0 ? ` · ${v.stock_damaged} damaged` : ""}</div>
              </div>
  ...
  return (
    <div className="flex flex-col" style={{ position: "fixed", inset: 0, zIndex: 30, background: C.ink }}>
      <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${C.inkLine}` }}>
        <div style={{ fontFamily: "Fraunces, serif", color: C.parchment, fontSize: 17, fontWeight: 600 }}>New Item</div>
      ...
      <div className="flex-1 px-5 py-5" style={{ background: C.inkSoft }}>
        <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Item name</div>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Tumbler" style={inputStyle} />
        <div style={{ color: C.ink40, fontSize: 12, marginTop: 8 }}>Add variants (sizes, colors) next — an item with no variants can't be sold yet.</div>
...
function EditVariantSheet({ item, variant, onClose, onSaved }) {
  ...
  return (
    <div className="flex flex-col" style={{ position: "fixed", inset: 0, zIndex: 30, background: C.ink }}>
      <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${C.inkLine}` }}>
        <div style={{ fontFamily: "Fraunces, serif", color: C.parchment, fontSize: 17, fontWeight: 600 }}>{variant ? "Edit" : "New"} Variant — {item.name}</div>
        <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer" }}><X size={20} color={C.ink40} /></button>
      </div>
      <div className="flex-1 px-5 py-5 flex flex-col gap-4" style={{ background: C.inkSoft }}>
        <div>
          <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Variant label <span style={{ color: C.ink40, fontWeight: 500 }}>(leave blank if none)</span></div>
          <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Red, Size M" style={inputStyle} />
        </div>
        <div>
          <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Price (IDR)</div>
          <input value={price} onChange={(e) => setPrice(e.target.value)} type="number" style={inputStyle} />
        </div>
        {!variant && (
          <div>
            <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Starting stock</div>
            <input value={stock} onChange={(e) => setStock(e.target.value)} type="number" style={inputStyle} />
          </div>
        )}
        {variant && <div style={{ color: C.ink40, fontSize: 12 }}>Stock is adjusted via sales, voids, or the damage-flag button — not edited here directly.</div>}
...
function DamageSheet({ variant, onClose, onSaved }) {
  ...
  return (
    <div className="flex flex-col" style={{ position: "fixed", inset: 0, zIndex: 30, background: C.ink }}>
      <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${C.inkLine}` }}>
        <div style={{ fontFamily: "Fraunces, serif", color: C.parchment, fontSize: 17, fontWeight: 600 }}>Adjust Condition</div>
        <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer" }}><X size={20} color={C.ink40} /></button>
      </div>
      <div className="flex-1 px-5 py-5 flex flex-col gap-4" style={{ background: C.inkSoft }}>
        <div style={{ color: C.ink60, fontSize: 12.5 }}>Available: {variant.stock_available} · Damaged: {variant.stock_damaged}</div>
        <div className="flex gap-2">
          <button onClick={() => setDirection("to_damaged")} className="flex-1 rounded-lg" style={{ padding: "10px 0", fontSize: 13, fontWeight: 700, background: direction === "to_damaged" ? C.gold : C.ink, color: direction === "to_damaged" ? C.ink : C.ink60, border: `1px solid ${C.inkLine}`, cursor: "pointer" }}>Mark as damaged</button>
          <button onClick={() => setDirection("to_available")} className="flex-1 rounded-lg" style={{ padding: "10px 0", fontSize: 13, fontWeight: 700, background: direction === "to_available" ? C.gold : C.ink, color: direction === "to_available" ? C.ink : C.ink60, border: `1px solid ${C.inkLine}`, cursor: "pointer" }}>Return to available</button>
        </div>
        <div>
          <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Quantity</div>
          <input value={quantity} onChange={(e) => setQuantity(e.target.value)} type="number" style={inputStyle} />
        </div>
        <div>
          <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Note <span style={{ color: C.ink40, fontWeight: 500 }}>(required)</span></div>
