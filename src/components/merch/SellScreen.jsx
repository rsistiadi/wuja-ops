...
export default function SellScreen({ crew, session }) {
  ...
  if (!session) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-2 px-8" style={{ background: C.inkSoft }}>
        ...
        <div style={{ color: C.parchment, fontSize: 15, fontWeight: 600, textAlign: "center" }}>No session open</div>
        <div style={{ color: C.ink40, fontSize: 13, textAlign: "center" }}>Open the day from the Session tab before making sales.</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-2.5" style={{ background: C.inkSoft }}>
      ...
      {items?.length === 0 && <div style={{ color: C.ink40, fontSize: 13.5 }}>No items in the catalog yet — ask an Admin to add some.</div>}
      {items?.map((item) => {
        ...
        return (
          <button key={item.id} onClick={() => setCheckout({ item, variant: variants.length === 1 ? variants[0] : null })}
            ...
            <div>
              <div style={{ color: C.parchment, fontSize: 15, fontWeight: 600 }}>{item.name}</div>
              <div style={{ color: C.ink40, fontSize: 12, marginTop: 3 }}>
  ...
  return (
    <div className="flex flex-col" style={{ position: "fixed", inset: 0, zIndex: 30, background: C.ink }}>
      <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${C.inkLine}` }}>
        <div style={{ fontFamily: "Fraunces, serif", color: C.parchment, fontSize: 17, fontWeight: 600 }}>{item.name}</div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-5" style={{ background: C.inkSoft }}>
        {variants.length > 1 && (
          <div>
            <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Variant</div>
            <div className="flex flex-col gap-1.5">
              {variants.map((v) => (
                <button key={v.id} onClick={() => setVariant(v)} disabled={v.stock_available === 0}
                  ...
                  <span style={{ color: C.parchment, fontSize: 14, fontWeight: 600 }}>{v.variant_label}</span>
                  <span style={{ color: C.ink40, fontSize: 12 }}>{v.stock_available === 0 ? "OUT" : `${v.stock_available} left`} · {formatIDR(v.price)}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {variant && (
          <>
            <div>
              <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Quantity <span style={{ color: C.ink40, fontWeight: 500 }}>({variant.stock_available} available)</span></div>
              <div className="flex items-center gap-3">
                ...
                <span style={{ fontFamily: "JetBrains Mono, monospace", color: C.parchment, fontSize: 22, fontWeight: 700, minWidth: 40, textAlign: "center" }}>{quantity}</span>
            ...
            <div>
              <div style={{ color: C.ink60, fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Payment method</div>
            ...
            <div className="rounded-xl px-4 py-3.5 flex items-center justify-between" style={{ background: C.ink, border: `1px solid ${C.gold}66` }}>
              <span style={{ color: C.ink60, fontSize: 13, fontWeight: 600 }}>Total</span>
              <span style={{ fontFamily: "JetBrains Mono, monospace", color: C.gold, fontSize: 20, fontWeight: 700 }}>{formatIDR(total)}</span>
            </div>
          </>
        )}
